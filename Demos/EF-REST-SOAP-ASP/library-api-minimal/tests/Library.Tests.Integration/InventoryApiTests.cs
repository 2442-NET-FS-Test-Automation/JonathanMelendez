...
public class InventoryApiTests
{
    ...
    {
        var login = await _client.PostAsJsonAsync("/auth/login", new {username = "jon", password = "qwerty123"});
    ...
    public async Task PostInventory_WithoutToken_Returns401()
    {
        ...
        var dto = new InventoryDTO("BK-401", "No Token", 1, 10.00m);
    ...
    public async Task PostInventory_AsAdmin_Creates()
    {
        ...
        var dto = new InventoryDTO("BK-TEST-INT", "Integration Test Book", 2, 21.50m);
    ...
    private record SupplierResponse(string sku, decimal supplierPrice);
}